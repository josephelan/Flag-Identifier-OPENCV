...
void filterRatios(std::list<std::string>& list,
  ...
  const float acceptable_error = image_ratio * 0.05;
  ...
  //  //Find corresponding image ratio
  //  float index_ratio = color_buckets.at(x).getCommonColorRatio();
  //    //Compare ratio (If ratios are not within acceptable range)
  //  if (index_ratio < min_ratio || index_ratio > max_ratio) {
  //    //Remove from results list
  //    //list.remove(x);
  //  }
...
void test(const std::unordered_map<int, std::unordered_map<int, std::unordered_map<int, std::list<std::string>>>>& flag_map,
  ...
  Mat test_file = imread("flags/wy.jpg");
...
int main(int argc, char* argv[]) {
  ...
  // Testing 
  test(flag_map, index_color_buckets);
